...
To configure leaderboard connectivity, set `RING_RUSH_CONVEX_URL`; no invitation code is needed for the public leaderboard. To also configure invited Daily in a local build, set `RING_RUSH_CONVEX_URL` and `RING_RUSH_CLOSED_TEST_CODE` in the build process environment. The builder creates the ignored `Assets/Resources/DailyConnection.asset`. It contains a tester invitation code, never a Convex administrative key. Guest tokens use iOS Keychain; Editor tokens remain in memory. Offline regular play works without this asset.
