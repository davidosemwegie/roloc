...
These public identifiers are saved in `Assets/Resources/AdsConfiguration.asset`. On September 8, 2026, the public [Ring Rush privacy policy](https://docs.google.com/document/d/1mDMFBulWr3so1DPojF6IYVHLHNd9H61ACp3FKwtMak0/view) was updated with advertising disclosures for build 10 and later while preserving the ad-free build 9 disclosure. Public access and the new text were verified without authentication, and the URL is now bundled in the configuration.
...
Publish an EEA/UK/Switzerland European-regulations message and the applicable US privacy-options message, including Unity Technologies SF (TCF vendor 1549) and ironSource (Additional Consent provider 2878). Only mark `ConsentMessagesPublished` true after verifying the published dashboard configuration; UMP's `canRequestAds` can otherwise return true even when no messages exist. The flag is true following dashboard publication verification on September 8, 2026. No language/timezone-based region guessing or production debug geography is used.

On September 8, 2026, **Ring Rush — European privacy choices** and **Ring Rush — US privacy choices** both showed **Published** for Ring Rush iOS. The European message targets EEA/UK/Switzerland and includes Consent, Do not consent, and Manage options; the US message targets all current and future supported US states with opt-out enabled. The policy URL is saved on the AdMob app. The account's existing common-partner list includes Unity Technologies SF (GVL 1549) and ironSource; no Google IDFA explainer was published. AdMob warns publication can take up to an hour to propagate.

The current [IAB vendor list](https://vendor-list.consensu.org/v3/vendor-list.json) (version 175, September 3, 2026) identifies Unity Technologies SF as vendor 1549. The native personalization gate now reads that vendor's `IABTCF_VendorConsents` bit instead of legacy AC provider 3234; a stale AC grant cannot override TCF refusal. ironSource still uses AC provider 2878. Storage consent, purposes 3/4, and ATT restrictions remain required as applicable.
...
LevelPlay reads actual UMP/Additional Consent decisions. ATT only adds a Unity Ads non-behavioral restriction; it never grants GDPR consent or fabricates a US sale/sharing choice. Under GDPR, tracking eligibility also requires personalization purposes 3/4 and consent for Unity Technologies SF (TCF vendor 1549) and ironSource (Additional Consent provider 2878). US regions requiring privacy options currently stay contextual because GPP opt-out decoding is not implemented. Inventory and late reward attempts are invalidated after provider privacy changes. Verify changes propagate to already initialized adapters on device: the SDK documentation does not specify their timing. LevelPlay has no shutdown API, and prior collection is not deleted by changing privacy options.
