...
Cosmetic progress, ordinary Flow/Rush traces and sound settings are not sent to PostHog. The analytics addition below sends selected usage events and run summaries through Convex; advertising SDKs separately process advertising analytics and diagnostics. Convex hosts the Daily and public leaderboard services. TestFlight can collect beta feedback and diagnostics under Apple's terms. Changing provider privacy options replaces app-controlled inventory and invalidates late rewards; it does not unload the native SDK or erase previously collected information. Verify actual updated consent propagation in the final device build.
...
## Player history and PostHog addition

Every online installation registers a Convex guest without requiring leaderboard participation or granting invitation-only Daily access. Convex stores a profile, analytics preference, and summaries of Flow, Rush, and Daily runs (including practice). Summaries contain a client run ID, mode, start/end timestamps, score, revive count, elapsed time, and started/completed/abandoned status. Ranked runs link to their owned ticket and retain a separate server validation outcome. These history records and profiles persist until operator deletion; the seven-day ticket purge and one-year best-score purge do not delete history.

Optional usage analytics is available only in eligible iOS release builds prepared for TestFlight/App Store distribution, on beta/production backends. Within those builds it is enabled by default and can be disabled in Settings. Editor, simulator, Mac, ordinary local and development builds do not send PostHog events, including server-generated ranking events. Convex profiles and run history continue in those builds, and the saved analytics preference is preserved. It sends selected gameplay interactions and run outcomes from Convex to PostHog in the CJVS Ring Rush project. PostHog receives a deployment-prefixed Convex user ID as its persistent identifier, event timestamps, mode, run/session IDs, score, revives, duration, and ranking outcomes. Nicknames, credentials, raw gameplay traces, email addresses, advertising identifiers, and device fingerprints are not included in these application payloads. GeoIP enrichment is disabled. This collection is linked to the installation identity; it is not anonymous aggregate-only collection.

Turning off usage analytics stops future PostHog collection after the preference reaches the server and discards older pending events. An event already in flight may complete. It does not delete previously delivered PostHog data or disable Convex game history, existing leaderboard records, advertising, or provider diagnostics. Offline preferences are persisted and sent before further telemetry uploads. Queued records retain their build eligibility, so upgrading from development to distribution does not backfill development analytics; legacy pending events without eligibility are discarded.

The local telemetry queue holds at most 256 pending items for seven days, discarding optional events first when full. Unsent records can be lost after prolonged offline use, storage failure, or credential loss. Convex keeps temporary event payloads for delivery for up to seven days and deduplication receipts for eight days. Delivered payloads are removed from the outbox. PostHog retention is controlled separately by the project's plan/settings and must be confirmed for the release; no automatic PostHog deletion is implemented here.

Update the published policy and App Store privacy purposes to include analytics before distributing this client. Preserve the historical build-9 disclosure above. The operator history deletion command in [ANALYTICS.md](ANALYTICS.md) covers run history and pending analytics; complete deletion also requires authentication, leaderboard records/aggregates, PostHog persons/events, and provider retention review.
